...
def create_request_folder(project_root, request_id):
    ...
    if os.path.exists(request_folder) and os.listdir(request_folder):
        print(
            f"\nWARNING: request_id '{request_id}' already exists and will be "
...
def read_scaled_bom_from_csv(bom_csv_path):
    scaled_bom = {}
    if not bom_csv_path or not os.path.exists(bom_csv_path):
        return scaled_bom

    with open(bom_csv_path, newline="") as csv_file:
        reader = csv.DictReader(csv_file)
        for row in reader:
            component = row.get("component_group") or row.get("Component Name")
            if not component:
                continue
            qty = row.get("new_qty")
            if qty is None:
                qty = row.get("qty")
            try:
                scaled_bom[str(component).strip()] = float(qty)
            except (TypeError, ValueError):
                scaled_bom[str(component).strip()] = 0.0

    return scaled_bom


def generate_step_cad_for_quote(request_id, sofa_type, dimensions_mm, output_dir=None):
    """Write a STEP CAD file for this quote. GLB is not a CAD deliverable."""
    if output_dir is None:
        output_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "outputs", "cad"))
    os.makedirs(output_dir, exist_ok=True)
    step_path = os.path.join(output_dir, f"{request_id}.step")
    try:
...
def main():
    ...
    cad_step = generate_step_cad_for_quote(request_id, sofa_type, dimensions_mm)
    quote_result["cad"] = cad_step
    if cad_step.get("step_path") and os.path.exists(cad_step["step_path"]):
        dst = os.path.join(request_folder, os.path.basename(cad_step["step_path"]))
        shutil.copy2(cad_step["step_path"], dst)
        copied_paths["step"] = dst
    copied_paths["step_path"] = cad_step.get("step_path")

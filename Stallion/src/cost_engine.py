...
# Schema import (optional — only used by generate_quote_from_schema)
try:
    from component_schema_extended import COMPONENT_SCHEMA_EXTENDED  # type: ignore
...
class SofaCostEngine:
    ...
    # ---------------------------------------------------
    # 11. SCHEMA-DRIVEN QUOTE (uses component_schema_extended instead of CSV BOM)
    # ---------------------------------------------------
    def generate_quote_from_schema(
        self,
        length_mm: float,
        width_mm: float,
        height_mm: float,
        sofa_type: str = "3-seater",
        output_prefix: str = "schema_quote",
    ) -> dict:
        """
        Generate a quote using the extended component schema as the data source
        instead of the CSV-based master_template_spec.

        The extended schema (component_schema_extended.py) defines base quantities
        and cost_per_unit for all 28 components.  This method applies the same
        scaling logic as generate_scaled_bom() but reads quantities from the schema
        rather than from master_template_spec.csv.

        Backward-compatible: generate_quote() (CSV-driven) is unchanged.

        Returns the same result dict shape as generate_quote() so callers can treat
        both interchangeably.
        """
        if not _SCHEMA_AVAILABLE:
            raise RuntimeError(
                "component_schema_extended is not importable. "
                "Cannot call generate_quote_from_schema()."
            )

        # We still need scale factors, so load dimension data if not yet loaded.
        if not self.TYPE_BASE_DIMS:
            self.master_dimensions = pd.read_csv(self.master_dim_path)
            for _, row in self.master_dimensions.iterrows():
                variant = str(row["variant"]).strip().lower()
                if not variant:
        ...
        scales = self.compute_scale_factors(length_mm, width_mm, height_mm, sofa_type)
        sofa_type_key = str(sofa_type).lower().strip()

        # ── Scale each schema component ────────────────────────────────────────
        scaled_rows = []
        springs_new = None

        # Two-pass: skip "derived from springs" on first pass
        for comp_id, comp_data in COMPONENT_SCHEMA_EXTENDED.items():
            base_qty = float(
                comp_data.get("base_qty_by_sofa_type", {}).get(sofa_type_key, 0.0)
            )
            if base_qty == 0:
                continue

            scaling_rule = comp_data.get("scaling_rule", "fixed")
            unit = comp_data.get("unit_of_measurement", "")
            cost_per_unit = float(comp_data.get("cost_per_unit", 0.0))

            if str(scaling_rule).strip().lower() == "derived from springs":
                continue
            ...
            if comp_id == "spring_unit":
                springs_new = new_qty

            scaled_rows.append({
                "component_group": comp_id,
                "base_qty": base_qty,
                "unit": unit,
                "scaling_rule": scaling_rule,
                "new_qty": new_qty,
                "unit_cost": cost_per_unit,
                "total_cost": new_qty * cost_per_unit,
                "notes": "",
            })

        # Second pass: derived components (spring_clip)
        for comp_id, comp_data in COMPONENT_SCHEMA_EXTENDED.items():
            base_qty = float(
                comp_data.get("base_qty_by_sofa_type", {}).get(sofa_type_key, 0.0)
            )
        ...
        # ── Cost summary (schema components carry their own unit costs) ─────────
        # Labour, finishing, overhead, profit come from cost_sheet.csv if loaded;
        # otherwise use the same defaults as the engine normally applies.
        labor_cost = 3500.0
        finishing_cost = 1200.0
        overhead_pct = 12.0
        profit_pct = 15.0

        if self.cost_sheet is not None:
            for _, row in self.cost_sheet.iterrows():
                if pd.isna(row.get("Component Name")):
                    continue
                name = str(row["Component Name"]).strip().lower()
                if name == "labour":
                    labor_cost = float(row["Value (INR or %)"])
        ...
        material_cost = float(bom_df["total_cost"].sum()) if not bom_df.empty else 0.0
        subtotal = material_cost + labor_cost + finishing_cost
        overhead = subtotal * (overhead_pct / 100.0)
        cost_after_overhead = subtotal + overhead
        profit = cost_after_overhead * (profit_pct / 100.0)
        ...
        bom_path = os.path.join(self.bom_output_dir, f"{output_prefix}_schema_bom.csv")
        quote_csv_path = os.path.join(self.quote_output_dir, f"{output_prefix}_schema_cost.csv")
        quote_json_path = os.path.join(self.quote_output_dir, f"{output_prefix}_schema_summary.json")

        bom_df.to_csv(bom_path, index=False)
        cost_df = bom_df[["component_group", "new_qty", "unit_cost", "total_cost"]].copy()
        cost_df.to_csv(quote_csv_path, index=False)
        with open(quote_json_path, "w") as f:
            json.dump(summary, f, indent=4)

        return {
            "source": "component_schema_extended",
            "input_dimensions": {
                "length_mm": length_mm,
                "width_mm": width_mm,
                "height_mm": height_mm,
